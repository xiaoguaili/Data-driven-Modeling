...
class NeuralNetwork:
    ...
    def __init__(self, X, Y, layers):
        ...
        # Normalize data
        self.Xmean, self.Xstd = X.mean(0), X.std(0) # Here 0 specifies the direction of mean
    ...
    def initialize_NN(self, Q):
        ...
        for layer in range(0,num_layers-1):
            ...
            #In the paper: Glorot, X., & Bengio, Y. (2010). Understanding the difficulty of training deep feedforward neural networks. In Proceedings of the Thirteenth International Conference on Artificial Intelligence and Statistics (pp. 249–256).
            #W follows uniform distribution from [-np.sqrt(6.0/(Q[layer]+Q[layer+1])), np.sqrt(6.0/(Q[layer]+Q[layer+1]))]
    ...
    def fetch_minibatch(self,X, Y, N_batch):
        ...
        # pick N_batch elements from N elements. There is no repeat.
        X_batch = X[idx,:]
        Y_batch = Y[idx,:]        
 ...
 #       y_star = y_star.flatten()*self.Ystd + self.Ymean
        return y_star
